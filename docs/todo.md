...
| R20 | **ext_store's CI builds a `v*` tag against the launcher's (autobleem) `master`, not the matching SDK tag** (found in R3, 2026-09-26): a release of the Store should build against the core/launcher version it ships with; today only the SDK-stamp check would catch a mismatch, loudly. Team: infrastructure. | ext_store CI | S | dev | a2 |
...
| D18 | **A native proofread of the translated manuals**: de, fr, es, it, pt-br (2026-09-26, Haiku translations checked only for structure) and the other ten as they land; per-language screenshots (`tools/manual_shots.py` LANGS) so the images match the text. Team: software/ui (screenshots); proofreading needs native speakers - owner / community. | manuals | M | owner + dev | rc |
| D19 | **Slim the launcher's CLAUDE.md** (the owner, 2026-09-26, from the PM's usage data): 154 KB (~40k tokens) is loaded into every session and every agent on every turn. Move the dated history narratives ("Current work", the NG port, the refactor phases, finished feature logs) to autobleem-main `docs/history/launcher-*.md` with a one-line pointer each; keep what a developer needs today (the code map, build, runtime layout, conventions, standing rules). Target <= 60 KB; nothing lost - every moved paragraph reachable from a pointer. Team: infrastructure (Ivy Morgan, Sonnet - judgment work; Nina Shaw checks nothing was lost), reviewed by Victor, Marcus told (his repo). | launcher, autobleem-main | M | dev | a2 |
