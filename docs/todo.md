...
|---|---|---|---|---|---|
| R1 | **Withdraw `v2.0.0-alpha2` and the stray `v2.0.0-alpha3`** (the owner, 2026-09-26: what we do now is still the road to the next alpha; alpha1 stays): delete both tags and their GitHub Releases in every repository that has them, take alpha2 off the site's testing channel (testing = alpha1 until the next pre-release), and the next pre-release is **alpha2** again. Before deleting: the nightly's name is the launcher's `git describe` (`v2.0.0-alpha2-N-g...`), which drops to `v2.0.0-alpha1-M-g...` - make sure the update check (launcher, installers, Imager lists, `repo_index.py`'s sort) does not see the next nightly as older than a device's current one, and that `release.py` skips or reuses an existing tag so a stray one cannot stop `promote` again. Team: infrastructure. | all component repos, site, release.py | M | dev | a2 |
| R2 | autobleem-build's master (`:latest`, what `v*` tags compile in) is 8 commits behind develop: SDL2 2.0.14 without OSS, no UPX for Windows exes, libdbus in the psc sysroot, the SignPath switch. Merge it (owner OK 2026-09-26), and make moving build's master a step of `promote` (it is not in `STAGES`). Team: infrastructure. | autobleem-build, release.py | S+M | dev | a2 |
...
| R18 | psc-kernel-payload's develop builds never reach the nightly (found by the build-times batch). Team: infrastructure. | psc-kernel-payload, appliance | S | dev | b1 |
...
| D5 | **autobleem-themes becomes the themes' source** (the owner, 2026-09-26): bring it up to the launcher's five themes (it holds stale copies of four), then the launcher/appliance take the themes from it (a release or pinned commit, like the other components) and the launcher's `payload/Themes` copy goes. `themes/default` stays wherever `Theme::load()` needs it at build and test time. Team: hardware. | themes, launcher, appliance | M | dev | a2 |
| D6 | **autobleem-samples becomes the sample pack's source** (the owner, 2026-09-26): move the launcher's `tools/samples/` + `build_samples.py` there, build the pack from it, delete the launcher's copy. Team: hardware. | samples, launcher | S-M | dev | a2 |
...
| D12 | Archive the finished launcher plans into the hub: quiet-stick, scanner-processors now; store, app-format, virtual-gamepad once their last row (H2/S6, S6, A3) closes. | launcher, hub | S | dev | a2 |
