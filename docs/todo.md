...
| C7 | **The launcher trusts the console's clock for the wrong reason.** `main.cpp` sets `Env::autobleemKernel` from `/autobleem` existing and `evoui_meta.cpp` then shows "Last Played" - but the AutoBleem kernel has no battery clock either: it boots at 2018-09-01 and is set only once a network is up (seen 2026-09-26 on the owner's console: timesyncd on systemd 229 never syncs without networkd; the payload now sets the time from a dhcpcd hook). Until then `time(nullptr)` is 2018 and every `updateDatePlayed` stores it (`Clock::displayTime` blanks < 2020, so it shows empty rather than wrong). Decide from the clock, not the kernel: e.g. a marker the payload's time hook writes once the time is set (`/run/autobleem/clock-set`), or "year >= the build's year"; record a play's date only then, re-checked each launch (the clock can be set mid-session). | launcher, payload | S | dev | a4 |
...
| ID | What | Where | Size | Who | Ms |
|---|---|---|---|---|---|
| K1 | ~~Merge the pad-driver kernel~~ - **done 2026-09-26** (owner's OK after the console tests): psc-kernel master = `6dd45f37`, psc-kernel-payload develop = `28f8756` (pad drivers, modules, abbtagent, WiFi hook, pairings kept, tzdata, the clock hook, `/tmp` not aged, WiFi+pairings kept over a flash, no pointer - its CLAUDE.md "On the console" has each), and autobleem-console-tools develop = `0d09025` (PSC-Bios's native backend: BlueZ over D-Bus, wpa_supplicant's socket, no shell; its step 5 partly run - `apps/pscbios/docs/native-backend-plan.md`). Left of K3's test list: a SHANWAN clone, DualSense/Switch Pro/Xbox over BT; of step 5: a wrong password, a cancelled pairing, bluetoothd stopped. | psc-kernel, psc-kernel-payload, console-tools | M | dev | a4 |
| K2 | Merge psc-kernel-payload `feature/userland-refresh` (docs: the owner's stay-on-4.4 decision, GE8300 not GX6250). | psc-kernel-payload | S | dev | a3 |
| K3 | Flash the fixed overlay on a console (the 2026-09-24 flash booted the kernel but AutoBleem did not start; the fix, `overlay.py shape/check`, was never flashed). Then the pad-driver test list: modules load, DS4 v2 BT, DS3 by cable through abbtagent, a SHANWAN clone, DualSense/Switch Pro/Xbox over BT, every pad in the launcher. | psc-kernel-payload | M | tester | a4 |
