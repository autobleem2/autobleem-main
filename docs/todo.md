...
| K9 | **dhcpcd's `env wpa_supplicant_driver` never reaches its hook** (found 2026-09-26 on the owner's console after the X6 flash): `/etc/dhcpcd.conf` has `env wpa_supplicant_driver=nl80211,wext` (global since X6; under `interface wlan0` before), the installed `10-wpa_supplicant` hook passes `-D$wpa_supplicant_driver` - yet wpa_supplicant runs as `wpa_supplicant -B -c/etc/wpa_supplicant.conf -iwlan0`, no `-D`, before and after X6. So it uses its first compiled driver (nl80211 - every in-kernel dongle, which is why nothing broke) and the wext fallback never applies; the old "Driver mode" row never took effect either. Suspect dhcpcd 9.4.1's PRIVSEP building the hooks' environment without `env` options - check with the `01-test` hook (`env > /tmp/...`), dhcpcd's `script.c`/privsep, or a hook-side default (`: ${wpa_supplicant_driver:=nl80211,wext}` in a `09-` hook). Matters only for a wext-only out-of-tree dongle. | psc-kernel-payload | S | dev | a4 |
