...
| `ext_store` | the AutoBleem Store, the first extension: a plugin the launcher loads from `Extensions/store/` (the launcher's `docs/extensions-plan.md`, `docs/store-plan.md`). An extension's repository is `ext_<name>`, an App's `app_<name>` | built with the launcher (`AB_EXTENSION_DIRS`) until the SDK package exists; a separate download, never in a release |
