...
| `autobleem-console-tools` | PSC-Bios (WiFi, timezone, pad wizard - an extension, `Extensions/pscbios/`, since 2026-09-24) and ABFlashKit (the kernel flasher, an App, carrying psc-kernel-payload's release) | `console-tools-psc-<v>.tar.gz` |
...
| `ext_store` | the AutoBleem Store, the first extension: a plugin the launcher loads from `Extensions/store/` (the launcher's `docs/extensions-plan.md`, `docs/store-plan.md`). An extension's repository is `ext_<name>`, an App's `app_<name>` | built with the launcher (`AB_EXTENSION_DIRS`) until the SDK package exists; shipped in every platform's package since 2026-09-25 (the appliance's `stage_extension`), and published on its own too |
| `proc_unzip` | the first scanner processor and the example to copy: unpacks zipped PS1 games and ROMs before every scan, from `System/Processors/unzip/` (the launcher's `docs/scanner-processors-plan.md`). One C++ file over miniz, no SDK. A processor's repository is `proc_<name>` | `unzip-<v>.zip` (every platform key in one folder), a rolling `nightly` release; its CI also runs the launcher's `tools/proc_check.py` |
